...
#         FILE:  Process.pm
#
#  DESCRIPTION:  Contains the calls to Gempak routines for processing BUFR files
...
%Bgruven = %main::Bgruven;
    ...
    if ($Bgruven{GRUVEN}->{OPTS}{noprocess} and %{$Bgruven{PROCESS}->{STATIONS}{process}}) {
        &Utils::modprint(6,9,96,1,2,"Your BUFR file processing is turned turned OFF - I hope that's what you wanted");
    } else {
...
#
    ...
    #
    foreach my $dir qw(gemdir ascdir) {
    ...
    foreach my $mod (sort keys %bufrs) {
      ...
      #     &Utils::modprint(1,5,144,2,1,"GEMPAK Aux Surface File: $gemaux") if $Bgruven{BINFO}->{GEMPAK}{packaux};
        ...
        while (my ($stnm,$bufr) = each %{$bufrs{$mod}}) {
            ...
            if (&Utils::execute("$exe < namsnd.in", $log)) {
                ...
                &Utils::rm($_) foreach ($gemsfc, $gemsnd, "$gemaux", "namsnd.in", "gemglb.nts", "last.nts");
                return (\@gemfls, \@ascfls);
            }
...
#   my $bufkitp = 'YYYYMMDDCC.MOD_STNM.buf';
    ...
    #
    my $work = "$bfkdir/work"; &Utils::rm($work);
    ...
    foreach my $mod (keys %{$Bgruven{PROCESS}->{STATIONS}{process}}) {
        ...
        #  Combine to create final BUFKIT file
        #
        chdir $work;
...
#           foreach (@lines) {tr/ / /s;s/^ //g;s/\n/\15\n/g;print OBFKT "$_";}
        ...
        if ($Bgruven{GRUVEN}->{OPTS}{zipit}) {
            ...
            #
            my %comp = ();
            foreach my $meth qw (zip gzip) {$comp{$meth} = &Utils::findcmd($meth);}
