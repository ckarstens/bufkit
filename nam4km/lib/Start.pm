...
#         FILE:  Start.pm
#
#  DESCRIPTION:  Contains the initialization and other general routines for bufrgruven
...
use Getopt::Long qw(:config pass_through);
    ...
    #
    foreach my $m qw (http ftp nfs) {push @{$gopts{methods}} => $m if grep /$m/i, @ARGV;}
...
#
    ...
    foreach (@_) {
        ...
        next if $_ eq '--help' ;
        next if $_ eq '--guide';
        next if /ver|zip|tidy|stnl/;  #  Skip zipit|verbose - The [no] part is problematic
