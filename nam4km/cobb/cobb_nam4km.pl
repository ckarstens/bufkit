...
#  $myFile = "/local/ckarsten/bufkit/modsnd/metdat/nam/"."$myModel"."_"."$myStation".".buf";
  $myFile = "/local/ckarsten/bufkit/nam4km/metdat/bufkit_temp/"."$myModel"."_"."$myStation".".buf";
  open(BUFIN,$myFile) or goto GETSTATION ; 
    if     ($myModel eq "eta" or $myModel eq "etam" or $myModel eq "etaw" or $myModel eq "nam4km" or $myModel eq "nmm" or $myModel eq "arw" or $myModel eq "ruc") { ReadEtaBufkit() }
...
#  if ($statime eq "")  { STDOUT->format_name("VER50"); STDOUT->format_top_name("VER50_TOP")}
  ...
  if     ($myStation eq "eta" )                        {$myModel = "eta"  ; print "Model now $myModel.\n" ;  goto GETSTATION}
   ...
   elsif ($myStation eq "nam4km")                         {$myModel = "nam4km"  ; print "Model now $myModel.\n" ;  goto GETSTATION}
                                                        ...
                                                        print ">change model - type one of these: eta, etam, nam4km, gfs, etaw, nmm, arw\n";
