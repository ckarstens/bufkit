...
#  $myFile = "//lan/bufkit_data/"."$myModel"."_"."$myStation".".buf";
  ...
  if ( $statime eq "" ) {
    ...
    for($i = 1; $i <= $#fcstHR; $i++) { 
       ...
       if ( $precip[$i] < 0.006 ) { if ( $myModel ne "gfs3") {$cumPrecipHours += 1.0 } else { $cumPrecipHours += 1.0 }} else { $cumPrecipHours = 0.0 } 
	   if ( $cumPrecipHours > 6.0 ) {$cumSnow = 0.0 ; $cumPrecip = 0.0 ; $cumSR = 0.0 ; $cumSleet = 0.0 ; $cumIce = 0.0 ; $cumSWE = 0.0 ; $cumPrecipHours = 0.0 ; }
       if ( $myModel ne "gfs3") {$cumHours += 1.0 } else { $cumHours += 1.0 }
	   ...
	   if ( $myModel eq "gfs3") { $i = $statime / 1 } else { $i = $statime }
       $snRatio[$i] = 0.0 ;
       $Ptype[$i] = GetPrecipType() ;           # Determine precipitation type.
	   GetSnow();
